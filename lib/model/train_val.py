...
class SolverWrapper(object):
    ...
    def find_previous(self):
        ...
        sfiles = [ss.replace('.meta', '')
                  for ss in sfiles if ss not in redfiles]

        nfiles = os.path.join(
            self.output_dir, cfg.TRAIN.SNAPSHOT_PREFIX + '_iter_*.pkl')
        nfiles = glob.glob(nfiles)
        nfiles.sort(key=os.path.getmtime)
        redfiles = [redfile.replace('.ckpt.meta', '.pkl')
                    for redfile in redfiles]
...
def train_net(network, imdb, roidb, valroidb, output_dir, tb_dir,
    ...
    #`allow_soft_placement`: If the devices you give don't exist, allow tensorflow distributes devices automatically.
