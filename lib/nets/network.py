...
class Network(object):
    def __init__(self):
        ...
        self._layers = {}
        self._gt_image = None
        self._act_summaries = []    # activations collected here
    ...
    def _add_score_summary(self, key, tensor):
        tf.summary.histogram('SCORE/' + tensor.op.name +
                             '/' + key + '/scores', tensor)
    ...
    def _crop_pool_layer(self, bottom, rois, name):
        with tf.variable_scope(name) as scope:
            ...
            height = (tf.to_float(
                bottom_shape[1]) - 1.) * np.float32(self._feat_stride[0])
            width = (tf.to_float(bottom_shape[2]) -
                     1.) * np.float32(self._feat_stride[0])
            ...
            crops = tf.image.crop_and_resize(bottom, bboxes, tf.to_int32(
                batch_ids), [pre_pool_size, pre_pool_size], name="crops")
    ...
    def _proposal_target_layer(self, rois, roi_scores, name):
        with tf.variable_scope(name) as scope:
            ...
            self._proposal_targets['labels'] = tf.to_int32(
                labels, name="to_int32")
    ...
    def _region_proposal(self, net_conv, is_training, initializer):
        ...
        rpn_cls_score_reshape = self._reshape_layer(
            rpn_cls_score, 2, 'rpn_cls_score_reshape')  # [bs, h*9, w, 2]
        ...
        if is_training:
            ...
            if cfg.TEST.MODE == 'nms':
                rois, _ = self._proposal_layer(
                    rpn_cls_prob, rpn_bbox_pred, "rois")
            elif cfg.TEST.MODE == 'top':
                rois, _ = self._proposal_top_layer(
                    rpn_cls_prob, rpn_bbox_pred, "rois")
            else:
                raise NotImplementedError
    ...
    def create_architecture(self, mode, num_classes, tag=None,
        ...
        # Number of anchors for each point in the feature map
